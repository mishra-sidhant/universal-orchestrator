...
Current canonical verification on July 12, 2026: the exact `uv run` gate passes with 198 tests without keys or provider sockets, Ruff passes, all built-in world-readiness eval cases pass, `mypy src` passes with strict mode and no disabled error codes, and the isolated sdist/wheel build succeeds. The opt-in provider smoke and real benchmark remain operator evidence by design.
