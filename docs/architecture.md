...
`BenchmarkRunner` executes the same invocation as two independently budgeted paths: one direct call to the strongest healthy configured model, then one complete orchestrated run. The comparison directory contains both outputs, both cost ledgers, latency, and the orchestrated quality/evidence reports. Its schema fixes `automated_superiority_claim` to null and delegates interpretation to human judgment; benchmark measurements do not rewrite configured capability priors automatically.
