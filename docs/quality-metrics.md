...
| `citation_support` | 0-100 integer | resolved claims with non-empty refs / completed-task claims | A claim resolves only when every ref is an existing chunk and was in that task's delivered context pack. It does not measure semantic entailment. |
...
- `cost_efficiency`: renamed to `routing_efficiency`; no provider usage/cost ledger exists yet.
